...
pub fn double_in_thread(numbers: Vec<i32>) -> Vec<i32> {
    ...
    let handle = thread::spawn(move || {
        numbers.into_iter().map(|i| i * 2).collect()
    });
    handle.join().unwrap()
}
...
pub fn parallel_sum(a: Vec<i32>, b: Vec<i32>) -> (i32, i32) {
    let h1 = thread::spawn(move || a.iter().sum());
    let h2 = thread::spawn(move || b.iter().sum());
    (h1.join().unwrap(), h2.join().unwrap())
}
...
pub fn named_sleeper(value: i32, ms: u64) -> i32 {
    ...
    let build = thread::Builder::new().name("sleeps".into());
    let handle = build.spawn(move || {
