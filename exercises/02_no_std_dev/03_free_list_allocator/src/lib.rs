...
unsafe impl GlobalAlloc for FreeListAllocator {
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        ...
        if prev_ptr != null_mut() && (*prev_ptr).size >= size && prev_ptr as usize % align == 0 {
            return prev_ptr as *mut u8;
        } else if prev_ptr != null_mut() {
            let mut curr = (*prev_ptr).next;
            while curr != null_mut() {
        ...
        loop {
            ...
            let new = (cur + align - 1) & !(align - 1);
            let end = new + size;
            if end > self.heap_end {
                return null_mut();
            }
            if self.bump_next.compare_exchange(cur, end, 
