...
impl Sv39PageTable {
    ...
    pub fn map_page(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        for level in (1..=2).rev() {
            ...
            let pte = {
                let node = self.nodes.get(&cur_ppn).unwrap();
                node.entries[vpn]
            };
            cur_ppn = if (pte & PTE_V) == 0 {
                let new_ppn = self.alloc_node();
                let node = self.nodes.get_mut(&cur_ppn).unwrap();
    ...
    pub fn translate(&self, va: u64) -> TranslateResult {
        ...
        for level in (0..=2).rev() {
            ...
            if pte & (PTE_R | PTE_W | PTE_X) > 0 && level > 0 {
                ...
                return TranslateResult::Ok(ppn << 12 | va & 0x1FFFFF);
            }
            if level == 0 {
                let ppn = (pte >> PPN_SHIFT) & PPN_MASK;
                return TranslateResult::Ok(ppn << 12 | va & 0xFFF);
            }
            cur_ppn = if (pte & PTE_V) == 0 {
