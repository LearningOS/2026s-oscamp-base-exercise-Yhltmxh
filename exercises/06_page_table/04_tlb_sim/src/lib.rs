...
impl Mmu {
    ...
    pub fn translate(&mut self, vpn: u64) -> Option<u64> {
        // TODO: 实现 TLB + 页表的二级查找
        let cur_asid = self.current_asid;
        match self.tlb.lookup(vpn, cur_asid) {
            Some(v) => Some(v),
            None => {
                for (asid, pm) in &self.page_table {
                    if *asid == cur_asid && vpn == pm.vpn {
                        self.tlb.insert(vpn, pm.ppn, cur_asid, pm.flags);
                        return Some(pm.ppn);
                    }
                }
                None
